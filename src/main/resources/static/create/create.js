...
// שימי לב: אין כאן getCookie או sendPost, אנחנו משתמשים באלו מ-helpers.js

function submitTask() {
    const responsibilityElements = document.querySelectorAll(".responsibility-field");
    const responsibilities = Array.from(responsibilityElements)
    ...
    .then(response => {
        if (response.ok) {
            ...
            alert("Failed to add task. Error: " + response.status);
        }
    })
    .catch(error => console.error("Error:", error));
}
