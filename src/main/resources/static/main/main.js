...
//             const fileOnly = String(comment.image).split(/[/\\]/).pop(); // מוריד תיקיות (גם / וגם \)
//             img.src = `${baseUrl}/image?img=${encodeURIComponent(fileOnly)}`;
...
//             // }
//             const att = String(comment.attachment).toLowerCase();
//             if (att.endsWith(".pdf")) {
//                 attachmentIcon.src = "/pdf-icon.png";
//                 attachmentIcon.alt = "Pdf";
//             }
//             else if (att.endsWith(".docx") || att.endsWith(".doc")) {
//                 attachmentIcon.src = "/doc-icon.jpg";
//                 attachmentIcon.alt = "Word Doc";
//             }
//             else {
...
//             const fileOnly = String(comment.attachment).split(/[/\\]/).pop();
//             attachmentLink.href = `${baseUrl}/attachment?file=${encodeURIComponent(fileOnly)}`;
//             attachmentLink.textContent = fileOnly;
...
async function fetchTasks() {
    try {
        // תיקון: שימוש בנתיב יחסי במקום baseUrl
...
function createComments(taskid, comments) {
    ...
    comments.forEach(comment => {
        ...
        // תיקון: תצוגת תמונה עם נתיב יחסי
        if (comment.image) {
            const img = document.createElement("img");
            const fileOnly = String(comment.image).split(/[/\\]/).pop(); // מוריד תיקיות
            // שינוי ל-Relative Path
            img.src = `/image?img=${encodeURIComponent(fileOnly)}`;
            img.alt = "Comment Image";
            commentElement.appendChild(img);
        }
        ...
        if (comment.attachment) {
            ...
            const fileOnly = String(comment.attachment).split(/[/\\]/).pop();
            // תיקון: קישור לקובץ עם נתיב יחסי
            attachmentLink.href = `/attachment?file=${encodeURIComponent(fileOnly)}`;
            attachmentLink.textContent = fileOnly;
